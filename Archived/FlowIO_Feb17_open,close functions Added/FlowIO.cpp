...
//API1: Two character based control. This is an implementation of our communication protocol at the library level. By invoking the "command()" 
	...
	float FlowIO::sendCommand(uint8_t action, uint8_t ports){ //returns the internal pressure
		...
		switch(action){
			...
			case '-':
				...
				break;
			case 'P': //inflate2x
				startInflation2x(ports); //these will not do anything unless _inf=true & _vac=false.
				if(_inf==true && _vac==false) pressure = getPressure();
				break;
			case 'N': //vacuum2x
				startVacuum2x(ports); //these will not do anything unless _vac=true & _inf=false.
...
//API2: functions to perform actions involving more than one element.
	...
	void FlowIO::startInflation2x(uint8_t ports){ //Only applicable for a DualInflation system.
	  	stopAction(ports);
	...
	void FlowIO::startVacuum2x(uint8_t ports){ //Only applicable for a DualVacuum system.
		stopAction(ports);
	...
	void FlowIO::stopAction(uint8_t ports){
		...
		//we want to CLOSE port 5. So we would actually have to invert the by bit that is 1. As for the bits that are
		//0, it means that we do not want to change those bits, which is different from setting then to 0. So what we want to
		//do is if we get a 1 bit we invert it, if we get a 0 bit, we keep the port unchanged. But how do I keep the port 
		//unchanged? The setPorts() function does not have this capability, so maybe I need to define a different function that has,
		//or I could just directly implement the functionality right here, without gooing throug a function. 
		//Alternatively, I could just have the stopAction() function act on all ports and not even thake any arguments.

		//Lets step back for a moment. If I send stopAction(0b0000001) I want to power off only last led and keep others unchanged.
		//I can add a second optional argument to the setPorts function that causes it to work only when something comes up to it.

		//I can have a function activatePorts() which sets HIGH only the ports having a 1 in the. Keeps others unchanged.
		//And a function deactivatePorts() which sets to 0 only the ports having a 1 in them. Keeps others unchanged.
		//And the function setPorts() which acts on all ports setting them high and low as needed.

		//In other words the first two functions would be used for addign or removing ports
