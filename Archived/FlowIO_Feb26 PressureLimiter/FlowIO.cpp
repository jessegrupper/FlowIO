	...
	float FlowIO::sendCommand(uint8_t action, uint8_t ports){ //returns the internal pressure
		...
		switch(action){
			...
			case '+':
				...
				if(_config == GENERAL || _config == INFLATION_SERIES || _config == INFLATION_PARALLEL) pressure = getPressure(); 
				break;
			case '-':
				startVacuum(ports);
				if(_config == GENERAL || _config == VACUUM_PARALLEL || _config == VACUUM_SERIES) pressure = getPressure();
				break;
			case 'p': //inflate2x
				startInflationHalfCapacity(ports);
				if(_config == INFLATION_SERIES || _config == INFLATION_PARALLEL) pressure = getPressure();
				break;
			case 'n': //vacuum2x
				startVacuumHalfCapacity(ports); 
				if(_config == VACUUM_SERIES || _config == VACUUM_PARALLEL) pressure = getPressure();
				break;
			case '^':
				startRelease(ports);
				pressure = getPressure();	
			...
			case '?':
				if(ports=='?'){
					...
					//TODO: I need to replace wthis stopAction with stopping of the individual components only. I must NOT be closing the ports. If I want to send repeated sense commands, 
					//currently this would just keep closing and reopening the ports, which is terrible for accuracy and also in terms of power. 
					stopAction(0xFF);
					setPorts(ports);
					pressure = getPressure();
					break;
