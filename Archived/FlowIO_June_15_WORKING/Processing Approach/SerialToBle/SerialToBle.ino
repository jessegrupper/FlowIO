...
BLEClientService clientPressureService = BLEClientService(UUID16_SVC_BLOOD_PRESSURE);
BLEClientCharacteristic clientPressureCharacteristic = BLEClientCharacteristic(UUID16_CHR_BLOOD_PRESSURE_MEASUREMENT);
...
void setup(){
  ...
  clientPressureService.begin();
  clientPressureCharacteristic.begin();
}
...
void connect_callback(uint16_t conn_handle){ //invoked when connection is established
  ...
  if (clientPressureService.discover(conn_handle)){
    Serial.print("Pressure Service Found! ");
    if(clientPressureCharacteristic.discover()){
      Serial.print("Initial Pressure is: ");
      float pressure; //this serves as our buffer for the first argument in next line.
      clientPressureCharacteristic.read(&pressure,sizeof(pressure));
...
void loop(){
  if(Bluefruit.Central.connected() && connectCallbackFinishedExecution==true){    
      ...
      //Refresh and send over serial the battery level every 100ms. (Refresh rate on FlowIO formware is 1000ms, so 100ms here is great.)
      if(millis() - batteryLevelReportedAt > 100){
        Serial.print("Batt:");
        Serial.println(clientBatteryService.read());
      }

      if(millis() - pressureReportedAt > 50){
        float pressure; //this serves as our buffer for the first argument in next line.
        clientPressureCharacteristic.read(&pressure,sizeof(pressure));
        Serial.print("Pres:");
        Serial.println(pressure);
