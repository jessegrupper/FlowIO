...
//DRIVER: functions to control individual elements
	...
	void FlowIO::startPump(uint8_t pumpNumber){			
		if(pumpNumber==1 || pumpNumber==2){
			...
			set_bit(6+pumpNumber,_hardwareState); //bit 7 is pump1, bit8 is pump2
		}
	}
	void FlowIO::startPump(uint8_t pumpNumber, uint8_t pwmValue){
		if(pumpNumber==1 || pumpNumber==2){
			...
			//update the _hardwareState variable
			set_bit(6+pumpNumber,_hardwareState); //bit 7 is pump1, bit8 is pump2
			//TODO: Change the definition of the _hardwareState byte to also show when PWM is used. Some bits are currently available
			//and can be defined to serve that purpose.
		}
	}												
