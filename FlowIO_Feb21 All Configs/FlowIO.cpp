	...
	float FlowIO::sendCommand(uint8_t action, uint8_t ports){ //returns the internal pressure
		...
		switch(action){
			...
			case '+':
				...
				if(_config == (GENERAL || INFLATION_SERIES || INFLATION_PARALLEL)) pressure = getPressure(); 
				break;
			case '-':
				startVacuum(ports);
				if(_config == (GENERAL || VACUUM_PARALLEL || VACUUM_SERIES)) pressure = getPressure();
				break;
			case 'p': //inflate2x
				startInflationHalfCapacity(ports);
				if(_config == (INFLATION_SERIES || INFLATION_PARALLEL)) pressure = getPressure();
				break;
			case 'n': //vacuum2x
				startVacuumHalfCapacity(ports); 
				if(_config == (VACUUM_SERIES || VACUUM_PARALLEL)) pressure = getPressure();
				break;
			case '^':
				startRelease(ports);
...
//PROGRAMMING API: functions to perform actions involving more than one element.
	void FlowIO::startInflation(uint8_t ports){ 
		stopAction(ports);
	  	if(_config == (GENERAL|| INFLATION_PARALLEL || INFLATION_SERIES)){
	...
	void FlowIO::startInflationHalfCapacity(uint8_t ports){ //Only applicable for a DualInflation system.
	  	stopAction(ports);
	  	if(_config == (INFLATION_PARALLEL || INFLATION_SERIES)){
	  		openInletValve(); 
	  		setPorts(ports);
	  		startPump(1);
	  	}
	...
	void FlowIO::startVacuum(uint8_t ports){ 
		stopAction(ports); 
		if(_config == (GENERAL || VACUUM_PARALLEL || VACUUM_SERIES)){
			openOutletValve();
			setPorts(ports);
			startPump(2);
	...
	void FlowIO::startVacuumHalfCapacity(uint8_t ports){ //Only applicable for a DualVacuum system.
		stopAction(ports);
		if(_config == (VACUUM_SERIES || VACUUM_PARALLEL)){
			openOutletValve();
			setPorts(ports);  
			startPump(1);
		}
