...
void setup() {
  ...
  cp5.addButton("Pres").setPosition(750,0).setSize(150,40);
    ...
    if(str != null){
      if(str.length() > 7){
        ...
        if(str.substring(0,5).equals("Pres:")){ 
          String pressureVal = str.substring(5,9); //returns positions 5 and 6 (starting position is 0).
          cp5.getController("Pres").setCaptionLabel(pressureVal);
        }
      }
    }
  }
