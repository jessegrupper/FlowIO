...
//  }
  ...
  if(uartService.available() >= MSG_SIZE){ 
    ...
    pressure = flowio.sendCommand(actionChar, portsChar);
      //we want to update the pressure only when a request for pressure is received. Otherwise, we don't read the pressure.
      //That is why we have the pressure update only inside this if block and nowhere else. But if we want the pressure to
      //update on some non-ble event, then we should have another if block that looks for that other event and inside of that
      //have a request for pressure update. 
  }
