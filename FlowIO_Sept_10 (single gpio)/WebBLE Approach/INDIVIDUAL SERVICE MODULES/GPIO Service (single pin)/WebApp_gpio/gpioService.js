...
const chrPin30UUID   = '0b0b0b0b-0b0b-0b0b-0b0b-c1000000aa06';
...
async function initGpioService() {
  try{
    ...
    chrPin30 = await gpioService.getCharacteristic(chrPin30UUID);

    //Subscribe to receive notifications about the pin value.
    await chrPin30.startNotifications();
    chrPin30.addEventListener('characteristicvaluechanged', event => { //an event is returned
      ...
      let pin30Val = event.target.value.getUint16(0,true); //The 0 means we are reading the lowest 16-bits. true means we flip the endicanness.
      log("pin30Val = " + pin30Val)
    })
  }
  catch(error){
...
async function writeDigital(pin,pinState){
  if (bleDevice && bleDevice.gatt.connected) {
    ...
    let value = new Uint8Array([pinState]);
    if(pin==30) await chrPin30.writeValue(value);
  }
...
async function writePWM(pin,pwmValue){
  if (bleDevice && bleDevice.gatt.connected) {
    ...
    let value = new Uint8Array([0x0A,pwmValue]);
    if(pin==30) await chrPin30.writeValue(value);
  }
  else log("Device not connected");
}
async function readDigital(pin,inputType){
  if (bleDevice && bleDevice.gatt.connected) {
    if(inputType=="INPUT")      value = new Uint8Array([0x02]);
    else if(inputType=="INPUT_PULLUP")      value = new Uint8Array([0x03]);
    if(pin==30) await chrPin30.writeValue(value);
    //It is not unnecessary to read the characteristic, b/c it sends a notification automatically.
    //that we are capturing already.
    //let pinVal = awaitchrPin30.readValue();
    //return pinVal;
  }
  else log("Device not connected");
}
async function readAnalog(pin,inputType){
  if (bleDevice && bleDevice.gatt.connected) {
    if(inputType=="INPUT") value = new Uint8Array([0x04]);
    else if(inputType=="INPUT_PULLUP") value = new Uint8Array([0x05]);
    if(pin==30) await chrPin30.writeValue(value)
    //It is not unnecessary to read the characteristic, b/c it sends a notification automatically.
    //that we are capturing already.
    //let pinVal = awaitchrPin30.readValue();
...
async function readAnalogNotify(pin,inputType){
  if (bleDevice && bleDevice.gatt.connected) {
    if(inputType=="INPUT") value = new Uint8Array([0x08]);
    else if(inputType=="INPUT_PULLUP") value = new Uint8Array([0x09]);
    if(pin==30) await chrPin30.writeValue(value)
    //It is not unnecessary to read the characteristic, b/c it sends a notification automatically.
    //that we are capturing already.
    //let pinVal = awaitchrPin30.readValue();
...
async function readDigitalNotify(pin,inputType){
  if (bleDevice && bleDevice.gatt.connected) {
    if(inputType=="INPUT") value = new Uint8Array([0x06]);
    else if(inputType=="INPUT_PULLUP") value = new Uint8Array([0x07]);
    if(pin==30) await chrPin30.writeValue(value)
    //It is not unnecessary to read the characteristic, b/c it sends a notification automatically.
    //that we are capturing already.
    //let pinVal = awaitchrPin30.readValue();
